...
describe('Dataset: Dataset crud', function () {
    ...
    let userId, appId, connectionId, datasetId,
        email = 'alberto.valle+cx021@qrvey.com',
    ...
    it('It should create a dataset from a custom sql query', async function () {
        this.timeout(5000);
        ...
        let dataSourceItem = {
            ...
            name: 'My Dataset',
            query: 'select `IATA_CODE`, `AIRLINE` from `airlines`',
        };
        ...
        expect(datasetItem.datasources[0]).to.have.property('query').to.be.a('string').to.be.eq(dataSourceItem.query);
